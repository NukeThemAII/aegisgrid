...
| Postgres persistence | Users, entitlements, credit ledger, payment event idempotency, and reports schema/repository | `db/schema.sql`, `src/lib/db/*` | `DATABASE_URL` | on demand | Parameterized queries only; DB errors fail closed for premium/report/payment flows | active foundation |
| Redis/shared cache | Optional Redis-backed JSON cache with in-memory TTL fallback | `src/lib/cache/cache-store.ts`, `/api/platform/status` | optional `REDIS_URL` | on demand | Redis URL is never exposed; cache values should contain public/feed data only; job queues still planned | active foundation |
| Stripe billing | Official `stripe` SDK Checkout, Portal, and raw-body webhooks | `/api/billing/checkout`, `/api/billing/portal`, `/api/billing/webhook` | `STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET`, Stripe price IDs, `DATABASE_URL` | webhook/on demand | Authenticated checkout only; webhooks verify Stripe signatures before parsing, reject live/test mismatch, use DB claim-before-process idempotency, no secrets logged | active foundation |
| x402 / USDC | Official x402 v2 `withX402` exact-EVM pay-per-report and pay-per-enrichment settlement plus admin audit lookup | `/api/x402/report`, `/api/x402/enrich`, `/api/x402/audit`, `@x402/next`, `@x402/evm`, `@coinbase/x402` | `DATABASE_URL`, `X402_ENABLED=true`, receiving address, facilitator URL, optional CDP creds, deterministic generation; audit lookup requires `AUTH_ADMIN_TOKEN` | on demand | No hand-rolled verification; paid routes fail closed when unconfigured, persist source-bounded result before settlement, settle only after successful handler response, record linked DB payment-event/ledger audit after settlement | active foundation |
...
- Premium/report/payment database paths use parameterized SQL through `pg`; if a configured DB errors, access/report persistence/payment fulfillment fails closed.
