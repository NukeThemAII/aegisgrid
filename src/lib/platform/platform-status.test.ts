...
describe('platform status', () => {
  ...
  it('summarizes configured services while redacting secret material', async () => {
    ...
    expect(status.database).toMatchObject({ status: 'configured', provider: 'postgresql' });
    ...
    expect(status.billing).toMatchObject({ premium_enabled: true, stripe_configured: true, x402_configured: true });
